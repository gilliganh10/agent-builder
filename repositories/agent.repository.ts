...
export const agentRepository = {
  ...
  findById: async (id: string): Promise<AgentDefinition | null> => {
    const row = await db.agentDefinition.findFirst({ where: { id } });
    return row ? rowToDefinition(row) : null;
  },

  findBySlug: async (slug: string): Promise<AgentDefinition | null> => {
  ...
  publish: async (id: string, token: string): Promise<AgentDefinition> => {
    const row = await db.agentDefinition.update({
      where: { id },
  ...
  unpublish: async (id: string): Promise<AgentDefinition> => {
    const row = await db.agentDefinition.update({
      where: { id },
